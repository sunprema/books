/* Two-page-spread pager for BookBank — The Memory Wall.
   Self-contained; works from file://. Exposes window.bookbankPager
   {next,prev,home}. The app binds -> <- ^ to it. Mirrors the reference
   pager in the write-book skill (padding-safe fold, click-routing). */
(function(){
  ...
  // or a mouse click jumps straight to the next file and skips the rest of
  ...
  // Copy-prompt affordance for image slots (plain-browser + app fallback).
  document.addEventListener('click', function(e){
    var b = e.target.closest && e.target.closest('.img-copy'); if(!b) return;
    var p = b.parentNode.querySelector('.img-prompt'); if(!p) return;
    navigator.clipboard && navigator.clipboard.writeText(p.textContent.trim());
    b.textContent = 'Copied'; setTimeout(function(){ b.textContent = 'Copy prompt'; }, 1200);
  });
})();
