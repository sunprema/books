(function(){
  ...
  // Image slots: "Copy prompt" buttons (plain-browser convenience).
  document.addEventListener('click', function(e){
    var b = e.target.closest && e.target.closest('.img-copy');
    if(!b) return;
    var slot = b.closest('.img-slot');
    var p = slot && slot.querySelector('.img-prompt');
    if(p && navigator.clipboard){ navigator.clipboard.writeText(p.textContent.trim()); b.textContent = 'Copied ✓'; setTimeout(function(){ b.textContent = 'Copy prompt'; }, 1600); }
  });
})();
