(function(){
  ...
  function layout(){
    ...
    // horizontal padding (the outer page margin), or two columns won't fit
    // and Chrome collapses the spread to one.
    var cs = getComputedStyle(leaf);
    var padX = (parseFloat(cs.paddingLeft) || 0) + (parseFloat(cs.paddingRight) || 0);
    var W = vp.clientWidth - padX, gap = Math.round(W * 0.07), colW = (W - gap) / 2;
  ...
  // "Copy prompt" for image slots (plain-browser convenience; app also wires this)
  document.addEventListener('click', function(e){
    var b = e.target.closest && e.target.closest('.img-copy');
    if(!b) return;
    var p = b.parentNode.querySelector('.img-prompt');
    if(p && navigator.clipboard){ navigator.clipboard.writeText(p.textContent.trim()); b.textContent = 'Copied ✓'; }
  });
})();
