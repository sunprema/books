/* Invisible Universe — book.js: the two-page-spread pager.
   Adapted from the write-book skill's reference implementation. Keeps every
   required contract: rel="next/prev/home", mobile fallback below ~900px,
   click-routing through the pager, bookbank:relayout dispatch, and (the
   `guitar`-book fix) subtracts the leaf's own horizontal padding before
   sizing spread columns so two columns + gap never overflow the box. */
(function(){
  ...
  function layout(){
    ...
    // Subtract the leaf's own horizontal padding (padding-left/right) before
    // sizing the two columns — otherwise two columns + gap can overflow the
    // content box (see validate_book.py's fold-padding check).
    var cs = getComputedStyle(leaf);
    var padL = parseFloat(cs.paddingLeft) || 0;
    var padR = parseFloat(cs.paddingRight) || 0;
    var W = vp.clientWidth - padL - padR;
    var gap = Math.round(W * 0.08), colW = (W - gap) / 2;
