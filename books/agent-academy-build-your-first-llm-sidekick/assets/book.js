(function(){
  ...
  function contentRight(){
    ...
    // child bounding rects rather than leaf.scrollWidth: rects are
    // engine-proof, and both rects carry the current translateX equally so
    // the difference is invariant while flipped or animating.
  ...
  function layout(){
    ...
    // Subtract the leaf's own horizontal padding, or two columns plus the
    // gap overflow the content box and the fold lands in the wrong place.
    var cs = getComputedStyle(leaf);
    var padL = parseFloat(cs.paddingLeft) || 0;
    var padR = parseFloat(cs.paddingRight) || 0;
    var W = vp.clientWidth - padL - padR;
    var gap = Math.round(W * 0.08);
    var colW = (W - gap) / 2;
    leaf.style.columnGap = gap + 'px';
    leaf.style.columnWidth = colW + 'px';
    spread = 2 * (colW + gap);
    var cols = Math.max(1, Math.ceil((contentRight() - padL - 1) / (colW + gap)));
  ...
  function render(){
    ...
    // book-progress.js listens for this to know the reader's spread.
  ...
  // sets window.__bookbankNav, so defer to it or the page turns twice.
  ...
  // with no scrollbar to reveal the loss.
  ...
  // Copy-prompt on image slots (plain-browser convenience; the app wires its own).
  document.addEventListener('click', function(e){
    var b = e.target.closest && e.target.closest('.img-copy');
    if(!b || !navigator.clipboard) return;
    var p = b.closest('.img-drop-inner').querySelector('.img-prompt');
