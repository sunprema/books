...
(function(){
  ...
  function render(){
    ...
    window.dispatchEvent(new CustomEvent('bookbank:spread', { detail:{ index:i } }));
  }
  ...
  // window.__bookbankNav), so defer to it to avoid turning twice. Also defer while
  // a widget's slider/number input has focus (e.g. the investment-game controls),
  // so editing a value doesn't accidentally flip the page.
  ...
  // skips the chapter's remaining spreads.
