...
(function(){
  ...
  function layout(){
    ...
    // horizontal padding, or two columns won't fit and the browser collapses
    // to one (content then overflows / overlaps the spread).
  ...
  // Plain-browser keyboard support; defer to the app when it hosts (sets __bookbankNav).
  ...
  // REQUIRED — route clicks on visible Next/Prev through the pager so a mouse
  // click steps through every spread before crossing into the next file.
  ...
  // "Copy prompt" for image slots (works in plain browser too).
  document.addEventListener('click', function(e){
