(function(){
  ...
  function contentRight(){
    // Rightmost content edge relative to the leaf's left edge, measured
    // from child bounding rects (engine-proof, unlike leaf.scrollWidth).
  ...
  // REQUIRED after every layout(): canvas widgets and 3D figures re-fit
  // and redraw on this event.
  ...
  layout();
  // Plain-browser keyboard support; the BookBank app sets window.__bookbankNav
  // and handles keys itself, so defer to it to avoid turning twice.
  ...
  // REQUIRED — route clicks on the visible Next/Prev links through the
  // pager so a click doesn't skip a chapter's remaining spreads.
